...
class PlanarSurfaceMultipleObjects : public rclcpp::Node
{
    public:
        PlanarSurfaceMultipleObjects(): Node("planar_grasping_multiple_objects")
        {
            ...
            cluster_topic_subscriber_ = this->create_subscription<clustering_itf::msg::StringArray>(
                "/cluster_topic_names", 10,
                std::bind(&PlanarSurfaceMultipleObjects::topic_callback, this, std::placeholders::_1));


            
            // for (const auto& topic_name : cluster_topic_names_)
            // {

            //     // Capture the topic_name for each subscription in the callback
            //     auto callback = [this, topic_name](const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
            //         this->cluster_callback(topic_name, msg);
            //     };
    ...
    private:
        ...
        void topic_callback(const clustering_itf::msg::StringArray::SharedPtr msg)
        {
            RCLCPP_INFO(this->get_logger(), "Received data from cluster_topic_names");
            //cluster_topic_names_ = msg.string_array;
                    // Clear existing subscriptions if necessary
            subscriptions_.clear();
            for (const auto& topic_name : msg->string_array)
            {
                RCLCPP_INFO(this->get_logger(), "Subscribing to: %s", topic_name.c_str());
        ...
        void cluster_callback(const std::string& topic_name, const sensor_msgs::msg::PointCloud2::SharedPtr msg)
        {
            RCLCPP_INFO(this->get_logger(), "Received data from: %s", topic_name.c_str());
            // Process the received point cloud data
            pcl::PointCloud<pcl::PointXYZ>::Ptr cloud(new pcl::PointCloud<pcl::PointXYZ>());
            pcl::fromROSMsg(*msg,*cloud);

            std::string top_of_object_topic = topic_name + "_top_of_object";
            std::string convex_hull = topic_name + "_convex";

            auto top_of_object_publisher_ = this->create_publisher<sensor_msgs::msg::PointCloud2>(top_of_object_topic,10);
            auto concave_hull_publisher_  = this->create_publisher<sensor_msgs::msg::PointCloud2>(convex_hull,10);
            ...
            RCLCPP_INFO(this->get_logger(), "Published to: %s and %s", top_of_object_topic.c_str(), convex_hull.c_str());
        ...
        rclcpp::Subscription<clustering_itf::msg::StringArray>::SharedPtr cluster_topic_subscriber_;
        ...
        std::vector<std::string> cluster_topic_names_;
